...
<div class="flex items-center justify-center min-h-screen px-6 bg-gradient-to-br from-slate-50 via-gray-50 to-blue-50">
    <div class="w-full max-w-md px-8 py-10 bg-white border border-gray-100 shadow-xl rounded-2xl">
        ...
        <!-- Error Message -->
        <?php $hasServerError = !empty($error); ?>
        <div id="error_message" class="<?= $hasServerError ? '' : 'hidden opacity-0' ?> p-3 mb-3 text-sm font-medium text-red-800 transition-all duration-300 ease-in-out border border-red-200 rounded-md bg-red-50">
            <div class="flex items-center">
                <svg class="flex-shrink-0 w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path>
                </svg>
                <span><?= $hasServerError ? $error : 'Invalid credentials. Try again.' ?></span>
            </div>
        </div>
